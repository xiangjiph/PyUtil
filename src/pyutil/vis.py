    ...
    for i, frame in enumerate(frames):
        fig.add_trace(
            go.Heatmap(
                ...
                zmin=zmin, zmax=zmax, 
                colorbar=dict(title="Intensity", titleside="right", titlefont=dict(size=12))
            )
        )
...
def scatter3(x=None, y=None, z=None, figsize=(600, 600), reverse_z_dir_Q=False, marker_size=1, aspectmode='cube', sub=None):
    if reverse_z_dir_Q:
        z_dir = "reversed"
    else:
        z_dir = True
